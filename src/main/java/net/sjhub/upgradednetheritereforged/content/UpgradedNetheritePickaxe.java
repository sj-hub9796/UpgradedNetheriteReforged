...
public class UpgradedNetheritePickaxe extends PickaxeItem {
   public UpgradedNetheritePickaxe(Tier tier, int attackDamageIn, float attackSpeedIn, Properties properties) {
      super(tier, attackDamageIn, attackSpeedIn, properties);
   }

   public boolean canApplyAtEnchantingTable(ItemStack itemStack, Enchantment enchantment) {
      return itemStack.getItem() == ModItems.CORRUPT_UPGRADED_NETHERITE_PICKAXE.get() && enchantment == Enchantments.MENDING ? false : enchantment.category.canEnchant(itemStack.getItem());
   }

   public void inventoryTick(ItemStack itemStack, Level level, Entity entity, int slot, boolean p_77663_5_) {
      if (itemStack.getItem() == ModItems.CORRUPT_UPGRADED_NETHERITE_PICKAXE.get() && itemStack.isEnchanted() && EnchantmentHelper.getTagEnchantmentLevel(Enchantments.MENDING, itemStack) > 0) {
